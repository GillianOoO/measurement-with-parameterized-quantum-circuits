...
Its `figs/` contains the six PDF filenames above. `response.tex` reuses the same
...
`validation/export_plot_bundle.py` exports the plotters, runner, validator and
instructions using an explicit allowlist. `bundle_manifest.json` records code
hashes; `required_inputs.json` records 91 required local input/asset filenames
and hashes without their numerical or audit contents. The optional
`--include-local-inputs` copies those files for local testing, not publication.
They must remain excluded from the GitHub branch unless separately approved for
release. Manuscript/reviewer documents, backups, caches and runtimes are excluded.
The tested workflow reconstructs figures from saved results; it is not a new
simulation. The full local archive retains experiment code, Hamiltonians,
checkpoints and measurement-replay records.
