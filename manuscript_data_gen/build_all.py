"""Rebuild current figures from published code and hash-verified bundled data."""
...
def verify_bundle(*, check_inputs=True):
    ...
    for relative, expected in files.items():
        ...
        if not source.is_file():
            raise FileNotFoundError(f"Missing input {relative}. Run python manuscript_data_gen/materialize_data.py first.")
...
def main():
    ...
    if not args.figures:
        subprocess.run([sys.executable,
                        str(BUNDLE / "code/analysis/rebuild_manuscript_tables.py"),
                        "--output", str(args.output.resolve() / "tables")], check=True)
